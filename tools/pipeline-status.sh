...
#
# Globs list every NAMED artifact a phase can legitimately produce — never a
# directory or a proxy that merely suggests the phase happened. A false positive
# is worse than a false negative here: it makes the workflow SKIP a phase that
# never ran. `docs/prfaq-<product>.md` is included because naming the PR/FAQ
# after the product is common and the workflow's own gate already globs
# `docs/prfaq*.md` — the scanner must not be stricter than the gate that reads it.
FOUNDATION_PHASES='product-idea|PR/FAQ (the bet)|docs/PRFAQ.md docs/prfaq.md docs/prfaq-*.md docs/PRFAQ-*.md
discovery|Discovery (validate the bet)|docs/discovery/00-discovery-brief.md docs/discovery/*.md
design-foundation|Design foundation|DESIGN.md docs/design-principles.md
